...
│   │   │   # Note: Actual tests remain in src/frontend/src/__tests__/
│   │   │
...
│       └── ... (all test files)
│
├── pyproject.toml                       # Updated: testpaths = ["qa/Backend"]
└── .github/
    └── workflows/
        └── ci.yml                       # Updated: test paths point to qa/Backend/
```
...
- **Location**: `src/frontend/src/__tests__/` (co-located with source)
...
1. ✅ Created categorized directory structure under `qa/`
2. ✅ Moved backend tests from `backend/tests/` to `qa/Backend/`
3. ✅ Organized backend tests into: Unit, Integration, Performance
4. ✅ Moved E2E tests from `qa/Tests/` to `qa/E2E/`
...
| File | Change |
|------|--------|
| `pyproject.toml` | `testpaths = ["qa/Backend"]` + `pythonpath = ["backend"]` |
| `src/frontend/vitest.config.js` | `include: ['src/**/*.test.{js,jsx,ts,tsx}']` |
| `src/frontend/playwright.config.js` | `testDir: '../QA/E2E'` |
| `.github/workflows/ci.yml` | Updated lint/test paths to `qa/Backend/` |
| `qa/Backend/conftest.py` | Added sys.path manipulation for `app` imports |
...
| File | Purpose |
|------|---------|
| `qa/README_STRUCTURE.md` | Complete structure and usage guide |
| `qa/TEST_INVENTORY.md` | Detailed test inventory with counts |
| `qa/Frontend/README.md` | Frontend test location explanation |
| `qa/QUICK_REFERENCE.md` | Quick commands reference |
| `qa/TEST_SUMMARY.md` | Test summary |
| `qa/STRUCTURE.md` | **(⭐ THIS FILE)** Visual tree and overview |
