...
async def create_share_link(
    ...
    return {"id": link_id, "resource": resource, "access_type": access_type, "expires_at": expires_at}

@router.get("/links", dependencies=[Depends(require_admin)])
async def list_share_links():
    return [
        {"id": l.id, "resource": l.resource, "access_type": l.access_type, "shared_with": l.shared_with, "expires_at": l.expires_at}
...
async def get_share_link(link_id: str):
    ...
    return {"id": link.id, "resource": link.resource, "access_type": link.access_type, "shared_with": link.shared_with, "expires_at": link.expires_at}
