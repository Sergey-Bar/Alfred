...
def list_compliance_tests():
    ...
    return [
        ComplianceTestResult(test_id="GDPR-001", status="passed", evidence_url="/evidence/gdpr-001.pdf", timestamp="2026-02-15T10:00:00Z"),
        ComplianceTestResult(test_id="SOC2-002", status="failed", evidence_url="/evidence/soc2-002.pdf", timestamp="2026-02-15T10:05:00Z")
    ]
...
def run_compliance_test(test_id: str):
    ...
    return ComplianceTestResult(test_id=test_id, status="passed", evidence_url=f"/evidence/{test_id}.pdf", timestamp="2026-02-15T10:10:00Z")

@router.get("/compliance/audit_logs", response_model=List[ComplianceAuditLog])
def export_audit_logs():
    # Stub: Return sample audit logs
    return [
        ComplianceAuditLog(log_id="log-001", action="test_run", user="admin", timestamp="2026-02-15T10:00:00Z", details="GDPR test executed"),
        ComplianceAuditLog(log_id="log-002", action="evidence_collected", user="admin", timestamp="2026-02-15T10:05:00Z", details="SOC2 evidence archived")
    ]
