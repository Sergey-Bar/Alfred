...
def query_analytics_events(event_type: Optional[str] = None, start: Optional[datetime] = None, end: Optional[datetime] = None, user: str = Depends(get_current_user)):
...
def aggregate_analytics(event_type: str, agg: str = "sum", user: str = Depends(get_current_user)):
    check_analytics_access(user, event_type)
    with Session(engine) as session:
        query = select(AnalyticsEventDB.value).where(AnalyticsEventDB.event_type == event_type, AnalyticsEventDB.value != None)
        values = [v[0] for v in session.exec(query).all()]
    result = {"count": len(values), "sum": sum(values) if values else 0, "avg": sum(values)/len(values) if values else None}
    if agg == "max":
...
def analytics_breakdown(by: str = "user", event_type: Optional[str] = None, user: str = Depends(get_current_user)):
    check_analytics_access(user, event_type)
    with Session(engine) as session:
        query = select(AnalyticsEventDB)
        if event_type:
...
@router.get("/analytics/events", response_model=List[AnalyticsEvent])
def query_analytics_events(event_type: Optional[str] = None, start: Optional[datetime] = None, end: Optional[datetime] = None):
...
def aggregate_analytics(event_type: str):
    values = [e.value for e in ANALYTICS_EVENTS if e.event_type == event_type and e.value is not None]
    if not values:
