...
# Logic: Secret Scanner for detecting API keys, passwords, and tokens in prompts.
# Why: Prevents accidental exposure of credentials to LLM providers.
# Root Cause: Developers may paste code containing secrets without realizing the risk.
...
class SecretScanner:
    ...
    PATTERNS: List[Tuple[str, SecretType, str, float]] = [
        ...
        # OpenAI
        (r"\bsk-[a-zA-Z0-9]{20,}T3BlbkFJ[a-zA-Z0-9]{20,}\b", SecretType.OPENAI_API_KEY, "OpenAI", 1.0),
        ...
        # Slack
        (r"\bxox[baprs]-[0-9]{10,13}-[0-9]{10,13}-[a-zA-Z0-9]{24}\b", SecretType.SLACK_TOKEN, "Slack", 1.0),
        ...
        # Database URLs
        (r"(?:postgres|mysql|mongodb|redis)(?:ql)?:\/\/[^\s]+:[^\s]+@[^\s]+", SecretType.DATABASE_URL, "Database", 1.0),
        
        # Private Keys
        (r"-----BEGIN (?:RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----", SecretType.PRIVATE_KEY, "Crypto", 1.0),
        (r"-----BEGIN PGP PRIVATE KEY BLOCK-----", SecretType.PRIVATE_KEY, "PGP", 1.0),
        
        # JWT Tokens
        (r"\beyJ[a-zA-Z0-9_-]*\.eyJ[a-zA-Z0-9_-]*\.[a-zA-Z0-9_-]*\b", SecretType.JWT_TOKEN, "JWT", 0.95),
        
        # Generic patterns (lower confidence)
        (r"\b(?:api[_-]?key|apikey)\s*[=:]\s*['\"]?([a-zA-Z0-9_-]{20,})['\"]?", SecretType.GENERIC_API_KEY, "Unknown", 0.7),
        (r"\b(?:secret|token)\s*[=:]\s*['\"]?([a-zA-Z0-9_-]{20,})['\"]?", SecretType.GENERIC_SECRET, "Unknown", 0.7),
        (r"\b(?:password|passwd|pwd)\s*[=:]\s*['\"]?([^\s'\"]{8,})['\"]?", SecretType.PASSWORD, "Unknown", 0.8),
    ]
