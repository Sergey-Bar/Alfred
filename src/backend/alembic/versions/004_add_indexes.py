...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_token_transfers_created_at"), "token_transfers", ["created_at"], unique=False
    )

    # Composite index for common query patterns
    op.create_index(
        "ix_request_logs_user_created", "request_logs", ["user_id", "created_at"], unique=False
    )
    ...
    op.create_index(
        "idx_approval_status_created", "approval_requests", ["status", "created_at"], unique=False
    )
