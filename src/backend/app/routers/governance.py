...
# [AI GENERATED]
# Model: GitHub Copilot (GPT-4.1)
# Logic: Adds /governance/transfer POST alias for test compatibility.
...
@_alias_router.post("/governance/transfer", response_model=TokenTransferResponse)
async def transfer_tokens_alias(
    transfer: TokenTransferRequest,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
):
    return await transfer_tokens(transfer, user, session)
...
async def transfer_tokens(
    ...
    credits_transferred_total.labels(from_user_id=str(user.id), to_user_id=str(recipient.id)).inc(
...
async def get_transfer_history(
    ...
    history = [format_transfer(t, True) for t in sent] + [
    ...
    try:
        if action == "approve":
            approval = mgr.approve(
                approval_id=approval_id, approver_id=str(user.id), approved_credits=approved_credits
            )
            ...
            if target:
                create_background_task(
                    emit_approval_resolved(
                        ...
                        approved=True,
                        credits=approval.approved_credits,
            ...
            if not approval:
                raise HTTPException(status_code=404, detail="WF not found")
...
async def get_leaderboard(
    period: str = "daily", limit: int = 10, session: Session = Depends(get_session)
):
    """Gamified Efficiency Ranking."""

    if period not in ["daily", "weekly", "monthly"]:
        raise HTTPException(
            status_code=400, detail="Invalid period: daily, weekly, or monthly required."
        )
    ...
    u_map = {
        u.id: u
        for u in session.exec(select(User).where(User.id.in_({e.user_id for e in entries}))).all()
    }
    ...
    return [
        LeaderboardResponse(
            ...
            user_name=u_map.get(e.user_id).name if u_map.get(e.user_id) else "Unknown",
            total_requests=e.total_requests,
            avg_efficiency_score=e.avg_efficiency_score,
...
async def get_usage_analytics(
    user: User = Depends(get_current_user), session: Session = Depends(get_session), days: int = 7
):
    ...
    for log in logs:
        ...
        if ds not in history:
            history[ds] = {"cost": Decimal("0"), "requests": 0, "tokens": 0}
        if log.model not in by_model:
            by_model[log.model] = {"cost": Decimal("0"), "requests": 0, "tokens": 0}
