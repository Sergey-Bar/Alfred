...
class PIIDetector:
    ...
    PATTERNS: Dict[PIIType, Tuple[str, str]] = {
        # (pattern, mask_format)
        PIIType.SSN: (r"\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b", "***-**-{last4}"),
        PIIType.CREDIT_CARD: (r"\b(?:\d{4}[-\s]?){3}\d{4}\b", "****-****-****-{last4}"),
        PIIType.EMAIL: (
        ...
        PIIType.IP_ADDRESS: (r"\b(?:\d{1,3}\.){3}\d{1,3}\b", "{first}.{second}.***.***"),
        PIIType.DATE_OF_BIRTH: (
            r"\b(?:0?[1-9]|1[0-2])[-/](?:0?[1-9]|[12]\d|3[01])[-/](?:19|20)\d{2}\b",
            "**/**/****",
        ),
        PIIType.PASSPORT: (r"\b[A-Z]{1,2}\d{6,9}\b", "**{last4}"),
        PIIType.BANK_ACCOUNT: (r"\b\d{8,17}\b", "****{last4}"),
    }
    ...
    ADDRESS_PATTERN = (
        rf"\b\d{{1,5}}\s+\w+(?:\s+\w+)*\s*,?\s*\w+\s*,?\s*{US_STATES}\s*\d{{5}}(?:-\d{{4}})?\b"
    )
    ...
    def _scan_with_presidio(self, text: str, seen_positions: set) -> List[PIIViolation]:
        """Use Presidio for NER-based PII detection."""
        violations = []

        try:
            results = self.presidio_analyzer.analyze(
                text=text, language="en", entities=None  # Detect all supported entities
            )
