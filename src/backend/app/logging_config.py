...
# Logic: This module centralizes all logging, tracing, and PII masking logic. It provides structured, context-aware logging for both production and development environments.
# Why: Structured logging is critical for observability, compliance, and debugging in distributed systems.
# Root Cause: Without structured/contextual logging, debugging and compliance are nearly impossible at scale.
# Context: All logging in the platform should use these helpers. Future: consider OpenTelemetry integration.
# Model Suitability: For logging patterns, GPT-4.1 is sufficient; for advanced distributed tracing, a more advanced model may be preferred.
"""
...
class JSONFormatter(logging.Formatter):
    # [AI GENERATED]
    # Model: GitHub Copilot (GPT-4.1)
    # Logic: Formats log records as JSON, recursively masks PII, injects context (request/user IDs).
    # Why: Ensures logs are machine-readable and safe for production ingestion.
    # Root Cause: Flat string logs are hard to parse and may leak secrets.
...
class DevelopmentFormatter(logging.Formatter):
    # [AI GENERATED]
    # Model: GitHub Copilot (GPT-4.1)
    # Logic: Formats logs for local development with color and context badges.
    # Why: Improves developer experience and traceability during debugging.
    # Root Cause: JSON logs are hard to read in the console.
    # Context: Used when log_format is not 'json'.
    """
    Console Beautifier.
...
class StructuredLogger(logging.Logger):
    # [AI GENERATED]
    # Model: GitHub Copilot (GPT-4.1)
    # Logic: Extends standard logger to support structured 'extra_data' for all log levels.
    # Why: Enables rich, queryable logs with business context.
    # Root Cause: Standard logging.Logger does not support structured extra fields.
    # Context: Used as the default logger class for the platform.
    """
    Enhanced Logger API.
...
def setup_logging(
    ...
    # Why: Ensures all logs are formatted, routed, and filtered correctly from startup.
    # Root Cause: Logging must be initialized before any app logic runs.
...
def get_logger(name: str) -> StructuredLogger:
    # [AI GENERATED]
    # Model: GitHub Copilot (GPT-4.1)
    # Logic: Returns a configured StructuredLogger for the given module or context.
    # Why: Ensures all modules use the same logging conventions and context.
    # Root Cause: Inconsistent logger usage leads to fragmented logs.
    # Context: Use everywhere logging is needed.
...
def log_request(
    ...
    # Why: Enables tracing, performance analysis, and debugging of API requests.
