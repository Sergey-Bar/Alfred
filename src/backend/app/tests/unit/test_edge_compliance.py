...
def test_proxy_quota_exceeded(test_client, admin_api_key):
    ...
    response = test_client.post("/v1/chat/completions", json={"tokens": 999999}, headers=admin_api_key)
...
def test_governance_invalid_token_transfer(test_client, admin_api_key):
    ...
    response = test_client.post("/v1/governance/transfer", json={"amount": -100}, headers=admin_api_key)
