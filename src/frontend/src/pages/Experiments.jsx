...
export default function Experiments() {
  ...
  useEffect(() => {
    ...
    api.getExperiments()
      .then(data => {
        if (mounted) {
          // API returns array or object with items property
          const items = Array.isArray(data) ? data : (data.items || data.experiments || []);
          setExperiments(items.length > 0 ? items : DEMO_EXPERIMENTS);
          setLoading(false);
        }
      })
      .catch(err => {
        if (mounted) {
          console.warn('Failed to fetch experiments, using demo data:', err.message);
