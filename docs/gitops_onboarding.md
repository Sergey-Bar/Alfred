...
## Repo Structure
- All deployment manifests are in `dev/devops/k8s/`
- Dockerfiles are in `dev/devops/docker/` and `docker-tools/`
- Secrets and config templates are in `dev/devops/secrets.md`
- Helm charts are in `dev/devops/charts/alfred/`
...
2. Add Alfred app:
   ...
   argocd app create alfred \
     ...
     --path dev/devops/k8s/production \
...
## Secrets Management
- Store secrets in cloud secret manager or sealed-secrets (see `dev/devops/secrets.md`)
...
- See `dev/devops/README.md` for advanced troubleshooting
