...
**User Stories:**
- *As a Finance Admin*, I want to allocate $50K/month to the Engineering department so I can control total AI spend
- *As an Engineering Manager*, I want to subdivide my department's credits across teams based on project priorities
- *As a Team Lead*, I want to give each developer a base allocation plus reserve a shared pool for the team
...
**User Stories:**
- *As a Developer*, I want to donate my unused credits to a teammate who's working on a deadline
- *As a Manager*, I want to temporarily boost a developer's quota for a critical sprint without changing base allocations
- *As a Developer*, I want to request credits from teammates when I've exhausted my quota
...
**User Stories:**
- *As a Developer*, when I set my status to "Vacation (2 weeks)", I want my credits to automatically help my team instead of going to waste
- *As a Manager*, I want to see a dashboard of OOO team members and where their credits went
- *As an Admin*, I want to configure org-wide policies: "Credits from OOO users go to team pool for 3 days, then return to department budget"
...
**User Stories:**
- *As a Team Lead*, I want to maintain a 500K token shared pool so my team has flexibility for spikes
- *As a Developer*, when I hit my personal quota, I want to automatically draw from the team pool if available
- *As a Finance Admin*, I want visibility into how much shared pool is being used vs individual allocations
...
**User Stories:**
- *As a Manager*, I want to grant a developer 2M extra tokens for 48 hours to handle a production incident
- *As a Developer*, I want to request an override with justification and have it approved in <5 minutes
- *As an Audit Team*, I want to see all overrides with timestamps, amounts, and business justification
...
- **Credit Utilization Rate**: (Used Credits / Allocated Credits) × 100
  - *Target: >85% (vs typical 60-70% without sharing)*
- **Cost Savings**: Reduction in wasted/unused credits
  - *Target: $10K+ saved per 100 users annually*
...
- **Pool Usage**: % of requests served by team pools
  - *Target: 15-25% of total requests*
...
1. **Credit Expiration**: Should unused credits expire monthly or roll over?
   - *Recommendation: Configurable per org, default to rollover with 90-day max*

2. **Transfer Reversibility**: Allow "undo" on transfers within X minutes?
   - *Recommendation: Yes, 5-minute window for accidental transfers*

3. **Anonymous Usage**: Should leaderboards/analytics support anonymous mode?
   - *Recommendation: Yes, opt-in per user*

4. **Multi-Currency**: Support multiple quota units (tokens, dollars, credits)?
   - *Recommendation: Yes, but normalize to tokens internally*

### 7.2 Technical Decisions
1. **Database**: PostgreSQL vs distributed DB (CockroachDB)?
   - *Recommendation: Start with PostgreSQL, migrate if >10K users*

2. **Caching Strategy**: Redis for what data?
   - *Recommendation: Quota balances, user sessions, rate limits*

3. **Real-Time Updates**: WebSockets vs Server-Sent Events vs polling?
   - *Recommendation: SSE for dashboard updates, WebSocket for notifications*

### 7.3 Go-to-Market
1. **Pricing Model**: Open core vs fully open source vs SaaS?
   - *Recommendation: Open core - core OSS, enterprise add-ons paid*

2. **Support Tiers**: Community vs paid support?
   - *Recommendation: Community Slack + paid enterprise support SLA*
...
Tier 2: Strategic Value (3-6 months post-launch)
3. WhatsApp ⭐⭐ STRATEGIC
...
Add within 6 months:
4. WhatsApp (if going global)
5. Webhooks (for custom integrations)
Evaluate based on demand:
6. Telegram (if crypto/web3 customers ask)
7. Discord (if gaming companies ask)
Skip:
8. Signal (too niche, limited API)
...
### **1. Prompt Safety & Content Filtering**
**The Problem:** Someone pastes your company's API keys, customer PII, or proprietary code into ChatGPT via Alfred. You're now liable.
...
│ 🔍 Scanning prompt...                    │
│                                           │
│ ⚠️ BLOCKED: PII detected                 │
│   - Email: customer@example.com          │
│   - SSN: ***-**-1234                     │
│   - Credit card: ****-****-****-5678     │
│                                           │
│ Recommendation: Redact before sending    │
│ [Redact & Send] [Edit] [Cancel]          │
└───────────────────────────────────────────┘
```

**Features:**
- **PII Detection**: SSNs, credit cards, emails, phone numbers, addresses
- **Secret Scanning**: API keys (AWS, OpenAI, etc.), passwords, tokens
- **Code Detection**: Proprietary file headers, internal package names
- **Prompt Injection Detection**: Block "ignore previous instructions" attacks
- **Jailbreak Detection**: Flag attempts to bypass model guardrails
- **Custom Blocklists**: Company-specific sensitive terms
...
**Why this matters:** One data leak kills your enterprise deal. Banks, healthcare, finance won't touch you without this.
...
If I could only add **3 things** to make Alfred unbeatable:

1. **Prompt Safety (PII/Secret Detection)** - Blocks enterprise sales without it
2. **Predictive Budgets + Waste Optimization** - Proves ROI, sells itself
3. **Quality Tracking + Model Benchmarking** - Answers "is AI actually helping?"

# Deep Dive: The 3 Killer Features
...
└─────────────────────────────────────────┘
       ...
       │             + Alert admin
       │
       ├─ REDACT ─> Auto-redact sensitive data
...
### 1.1 PII Detection Engine

**Libraries:**
- **Presidio** (Microsoft) - Best open-source PII detector
- **spaCy** - NER (Named Entity Recognition)
- **Regex patterns** - Fallback for structured data

**Implementation:**

```python
# backend/app/safety/pii_detector.py
...
class PIIDetector:
    def __init__(self):
        ...
        # Custom patterns for company-specific PII
        self.custom_patterns = {
            'employee_id': r'EMP-\d{6}',
            'internal_ticket': r'JIRA-\d{4,6}',
            'api_key': r'sk-[a-zA-Z0-9]{48}',
            'aws_key': r'AKIA[0-9A-Z]{16}',
        }
    
    def detect(self, text: str, org_id: str) -> Dict:
        """
        Detect PII in text.
        ...
        # 1. Built-in PII detection (Presidio)
        ...
        for result in results:
            findings.append({
                ...
                'end': result.end,
                'score': result.score,
                'severity': self._get_severity(result.entity_type)
            })
        ...
        # 3. Org-specific patterns (from DB)
        org_patterns = self._get_org_patterns(org_id)
        for pattern_config in org_patterns:
            matches = re.finditer(pattern_config['regex'], text)
            for match in matches:
                findings.append({
    ...
    def _get_severity(self, entity_type: str) -> str:
        ...
        critical = ['CREDIT_CARD', 'US_SSN', 'US_PASSPORT', 'CRYPTO']
        high = ['EMAIL_ADDRESS', 'PHONE_NUMBER', 'IBAN_CODE', 'MEDICAL_LICENSE']
        medium = ['PERSON', 'LOCATION']
        low = ['DATE_TIME']
    ...
    def _get_mask(self, finding: Dict) -> str:
        """Generate appropriate mask for redacted data."""
        masks = {
            'EMAIL_ADDRESS': '[EMAIL_REDACTED]',
            'CREDIT_CARD': '[CARD_****' + finding['value'][-4:] + ']',
            'US_SSN': '[SSN_***-**-****]',
            'PHONE_NUMBER': '[PHONE_REDACTED]',
            'CRYPTO': '[CRYPTO_WALLET_REDACTED]',
            'PERSON': '[NAME_REDACTED]',
            'API_KEY': '[API_KEY_REDACTED]',
        }
        return masks.get(finding['type'], '[REDACTED]')
...
```python
# backend/app/safety/secret_scanner.py

import re
from typing import List, Dict
import entropy  # For high-entropy string detection
...
class SecretScanner:
    def __init__(self):
        self.patterns = {
            ...
            # Private keys
            'rsa_private': r'-----BEGIN (RSA|OPENSSH|DSA|EC) PRIVATE KEY-----',
            'pgp_private': r'-----BEGIN PGP PRIVATE KEY BLOCK-----',
            
            # Database connections
            'db_connection': r'(mysql|postgres|mongodb)://[^\s]+',
    ...
    def scan(self, text: str) -> Dict:
        ...
        Returns similar structure to PII detector.
        """
        findings = []
        ...
        high_entropy_strings = self._find_high_entropy(text)
        findings.extend(high_entropy_strings)
        
        # 3. Known leaked secrets (check against DB)
        known_leaks = self._check_known_leaks(text)
        findings.extend(known_leaks)
        
        return {
            'has_secrets': len(findings) > 0,
            'findings': findings,
            'should_block': len(findings) > 0,  # Always block secrets
            'redacted_text': self._redact_secrets(text, findings)
        }
    
    def _find_high_entropy(self, text: str) -> List[Dict]:
        """
        Find strings with high Shannon entropy (likely secrets).
        Entropy > 4.5 for base64-like strings often indicates secrets.
        """
        findings = []
        # Split into words
        ...
        for word in words:
            ent = self._shannon_entropy(word)
            if ent > 4.5:  # Threshold for suspicion
                findings.append({
                    'type': 'high_entropy_string',
                    'value': word[:10] + '...',  # Don't log full value
    ...
    def _check_known_leaks(self, text: str) -> List[Dict]:
        """
        Check against database of known compromised secrets.
        Integrate with services like:
        - GitGuardian
        - HaveIBeenPwned API
        - Internal breach database
        """
        # TODO: Implement when breach DB available
        return []
    
    def _redact_secrets(self, text: str, findings: List[Dict]) -> str:
        """Replace all secrets with [SECRET_REDACTED]."""
        redacted = text
        for finding in sorted(findings, key=lambda x: x['start'], reverse=True):
            redacted = (
                redacted[:finding['start']] + 
...
```python
# backend/app/safety/prompt_injection.py
...
class PromptInjectionDetector:
    def __init__(self):
        ...
        self.patterns = [
            ...
            r'disregard\s+(previous|all|prior)\s+(instructions?|rules?|directives?)',
            r'forget\s+(everything|all|previous)',
            r'you\s+are\s+now',
            r'new\s+instructions?:',
            r'system\s+prompt\s+override',
            r'admin\s+mode\s+(on|enabled|activate)',
            r'developer\s+mode',
            r'sudo\s+mode',
            r'\\n\\nSystem:',  # Trying to inject system message
        ...
        self.suspicious_keywords = [
            'jailbreak', 'dan mode', 'based mode',
            'roleplay as', 'pretend you are',
            'act as if', 'bypass', 'override'
        ]
    ...
    def detect(self, text: str) -> Dict:
        ...
        # Excessive special characters, encoding attempts
        ...
        # 4. Repetition patterns (common in jailbreaks)
        # "Please please please please..."
        repetitions = self._find_repetitions(text)
        if repetitions > 5:
            findings.append({
        ...
        return {
            ...
            'should_block': risk_score > 70,  # Threshold
            'should_warn': 40 < risk_score <= 70
        }
    
    def _find_repetitions(self, text: str) -> int:
        """Count repeated word patterns."""
...
from ..models import Organization, User, SafetyPolicy
from typing import Dict, List
import asyncio
...
class SafetyPipeline:
    ...
    async def analyze(
        ...
        """
        # Get org safety policy
        policy = await self._get_policy(org_id)
        
        # Run checks in parallel
        pii_task = asyncio.create_task(
            asyncio.to_thread(self.pii_detector.detect, text, org_id)
        )
        secrets_task = asyncio.create_task(
            asyncio.to_thread(self.secret_scanner.scan, text)
        )
        injection_task = asyncio.create_task(
            asyncio.to_thread(self.injection_detector.detect, text)
        )
        ...
        if pii_result['has_pii']:
            ...
            elif policy.warn_on_pii:
                warnings.append({
                    'type': 'pii_detected',
                    'details': pii_result['findings'],
                    'message': 'PII detected - consider removing'
                })
        
        if secrets_result['has_secrets']:
            violations.append({
                'type': 'secrets_detected',
                'details': secrets_result['findings'],
                'message': 'Secrets/credentials detected'
            })
        
        if injection_result['should_block']:
            violations.append({
                'type': 'prompt_injection',
                'details': injection_result['findings'],
                'message': 'Potential prompt injection detected'
            })
        elif injection_result['should_warn']:
            warnings.append({
                'type': 'prompt_injection',
                'details': injection_result['findings'],
                'message': 'Suspicious prompt patterns detected'
            })
        
        # Calculate overall risk
        overall_risk = max(
            pii_result.get('risk_score', 0),
            secrets_result.get('risk_score', 100) if secrets_result['has_secrets'] else 0,
            injection_result.get('risk_score', 0)
        )
        ...
        redacted_text = text
        if action == 'redact' and policy.auto_redact:
    ...
    def _determine_action(
        ...
        if violations:
            ...
            if any(v['type'] == 'secrets_detected' for v in violations):
                return 'block'
            
            # PII handling depends on policy
            if any(v['type'] == 'pii_detected' for v in violations):
                if policy.auto_redact:
                    return 'redact'
                elif policy.block_on_pii:
                    return 'block'
            
            # Prompt injection
            if any(v['type'] == 'prompt_injection' for v in violations):
                return 'block'
        
        if warnings and policy.show_warnings:
            return 'warn'
        
        return 'allow'
    
    async def _get_policy(self, org_id: str) -> SafetyPolicy:
        """Fetch org safety policy from DB."""
...
from ..models import ProxyRequest, SafetyCheckResult
...
async def chat_completions(
    ...
    """
    OpenAI-compatible endpoint with safety checks.
    """
    ...
    # Run safety pipeline
    safety_result = await safety_pipeline.analyze(
        text=full_prompt,
        user_id=user.id,
    ...
    # Log the check
    await log_safety_check(user.id, safety_result)
    
    # Handle result
    if safety_result['action'] == 'block':
        logger.warning(
            f"Request blocked for user {user.id}",
            extra={'violations': safety_result['violations']}
        )
        
        # Notify admin for critical violations
        if any(v['type'] == 'secrets_detected' for v in safety_result['violations']):
            await notify_security_team(user, safety_result)
        ...
        raise HTTPException(
            ...
            detail={
                ...
                'help': 'Please remove sensitive data and try again'
            }
        )
    
    elif safety_result['action'] == 'redact':
        logger.info(f"Auto-redacting PII for user {user.id}")
        # Replace prompt with redacted version
        request.messages[-1]['content'] = safety_result['redacted_text']
    
    elif safety_result['action'] == 'warn':
        # Continue but add warning header
        pass
    
    # Continue to quota check and LLM proxy...
    # (existing Alfred logic)
```

## Frontend UI Components

### 1.6 Safety Alert Modal

```jsx
// src/frontend/src/components/SafetyAlert.jsx
      ...
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600 mt-1">
              Your prompt contains sensitive information that violates company policy
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {violations.map((violation, idx) => (
            <div 
              key={idx}
              className={`p-4 rounded-lg border-l-4 border-${getSeverityColor(violation.severity)}-500 bg-${getSeverityColor(violation.severity)}-50`}
            >
              <div className="flex items-start gap-2">
                <AlertTriangle className={`text-${getSeverityColor(violation.severity)}-600 mt-0.5`} size={16} />
                <div className="flex-1">
                  ...
                  {violation.details && violation.details.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {violation.details.map((detail, i) => (
                        <div key={i} className="text-sm font-mono bg-white px-2 py-1 rounded">
                          {detail.type}: <span className="text-red-600">{detail.value}</span>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="flex gap-2">
            ...
            <div className="text-sm text-blue-900">
              ...
              <ul className="mt-2 space-y-1 list-disc list-inside">
                <li>Click <strong>Auto-Redact</strong> to automatically remove sensitive data</li>
                <li>Click <strong>Edit Prompt</strong> to manually fix the issues</li>
                <li>If this is an error, contact your administrator</li>
              </ul>
            </div>
          </div>
        </div>
...
### 1.7 Admin Safety Dashboard

```jsx
// src/frontend/src/pages/SafetyDashboard.jsx
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Monitor and manage AI safety across your organization</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-left py-3">Violation</th>
          ...
          <tbody>
            {incidents.map((incident, idx) => (
              <tr key={idx} className="border-b hover:bg-gray-50">
                <td className="py-3 text-sm">{incident.timestamp}</td>
                <td className="py-3 text-sm">{incident.user}</td>
                <td className="py-3 text-sm font-mono">{incident.type}</td>
                <td className="py-3">
                  <span className={`px-2 py-1 rounded text-xs font-medium bg-${getSeverityColor(incident.severity)}-100 text-${getSeverityColor(incident.severity)}-800`}>
                    {incident.severity}
                  </span>
                </td>
                <td className="py-3 text-sm">{incident.action}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
CREATE TABLE safety_incidents (
    ...
    -- Detection results
    violation_type VARCHAR(50), -- 'pii', 'secrets', 'injection'
    severity VARCHAR(20), -- 'critical', 'high', 'medium', 'low'
    findings JSONB, -- Detailed findings
    
    -- Action taken
    action VARCHAR(20), -- 'block', 'redact', 'warn', 'allow'
    redacted BOOLEAN DEFAULT FALSE,
    
    -- Context
    original_prompt_hash VARCHAR(64), -- SHA256 hash (don't store actual prompt)
    metadata JSONB,
...
CREATE TABLE safety_policies (
    ...
    pii_types_to_detect TEXT[], -- e.g., ['EMAIL', 'SSN', 'CREDIT_CARD']
    ...
    injection_threshold INTEGER DEFAULT 70, -- Risk score threshold
    
    -- Custom patterns
    custom_patterns JSONB, -- Org-specific patterns
...
This is Part 1 of the deep dive (Prompt Safety). 
...
import React, { useState, useEffect } from 'react';
import { Star, TrendingUp, ThumbsUp, ThumbsDown, MessageSquare, Award, BarChart3 } from 'lucide-react';
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-600">Track response quality, user satisfaction, and model performance</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-blue-100 text-sm mb-2">Overall AI Quality Score</p>
            <div className="flex items-center gap-3">
              <span className="text-5xl font-bold">{metrics.overall_rating}</span>
              <div className="flex">
                {[1, 2, 3, 4, 5].map(i => (
                  <Star
                    key={i}
                    size={28}
                    className={i <= Math.round(metrics.overall_rating) ? 'fill-yellow-300 text-yellow-300' : 'text-blue-300'}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="text-right">
            <div className="inline-flex items-center gap-2 bg-white/20 rounded-lg px-4 py-2 mb-3">
              ...
              <span className="text-lg font-semibold">
                {metrics.trending === 'improving' ? '↑ Improving' :
                 metrics.trending === 'declining' ? '↓ Declining' :
                 '→ Stable'}
              </span>
            </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="font-semibold text-blue-900 mb-3">Want to improve quality?</h3>
        <div className="flex gap-2">
...
const RatingDistributionChart = ({ data }) => {
  const chartData = Object.entries(data).map(([stars, count]) => ({
    stars: `${stars} ⭐`,
    count: count,
          ...
          content={({ active, payload }) => {
            ...
            return (
              <div className="bg-white border rounded-lg shadow-lg p-2">
                <p className="text-sm font-medium">{payload[0].payload.stars}</p>
                <p className="text-sm text-gray-600">{payload[0].value} ratings ({payload[0].payload.percentage.toFixed(1)}%)</p>
              </div>
            );
          }}
        />
      ...
      <PieChart>
        ...
        >
          {chartData.map((entry, index) => (
            <Cell key={`cell-${index}`} fill={COLORS[entry.name.replace(' ', '_')] || '#3b82f6'} />
          ))}
        </Pie>
        <Tooltip />
      </PieChart>
...
const ModelComparisonTable = ({ models }) => {
  const modelEntries = Object.entries(models).sort((a, b) => b[1].avg_rating - a[1].avg_rating);
        ...
        <tbody>
          {modelEntries.map(([model, stats], idx) => (
            <tr key={model} className="border-b hover:bg-gray-50">
              ...
              <td className="py-3 px-4">
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{stats.avg_rating.toFixed(2)}</span>
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map(i => (
                      <Star
                        key={i}
                        size={14}
                        className={i <= Math.round(stats.avg_rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}
                      />
                    ))}
                  </div>
                </div>
              </td>
              <td className="py-3 px-4 text-gray-700">{stats.count.toLocaleString()}</td>
              <td className="py-3 px-4">
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  stats.helpful_rate > 0.8 ? 'bg-green-100 text-green-800' :
                  stats.helpful_rate > 0.6 ? 'bg-yellow-100 text-yellow-800' :
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-orange-50 border border-orange-200 rounded-lg">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="font-medium text-gray-900 capitalize">{issue.issue.replace('_', ' ')}</p>
...
import React, { useState, useEffect } from 'react';
import { Play, Clock, DollarSign, Award, TrendingUp, AlertCircle } from 'lucide-react';
...
export const ModelBenchmarkPage = () => {
  ...
  const loadBenchmarks = async () => {
    const data = await fetch('/api/v1/quality/benchmarks').then(r => r.json());
    setBenchmarks(data);
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Compare models for your specific use cases</p>
        </div>
        <button
      ...
      {benchmarks.filter(b => b.status === 'running').length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="font-semibold text-blue-900 mb-3">Running Benchmarks</h3>
          <div className="space-y-2">
            {benchmarks.filter(b => b.status === 'running').map(benchmark => (
              <RunningBenchmarkCard key={benchmark.id} benchmark={benchmark} />
            ))}
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {benchmarks.filter(b => b.status === 'completed').map(benchmark => (
    ...
    <div className="bg-white border rounded-lg p-4 flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">Testing {benchmark.models.length} models...</p>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-green-900">Recommended: {recommendation.recommended_model}</p>
              <p className="text-sm text-green-800 mt-1">{recommendation.reason}</p>
            </div>
          </div>
        </div>
          ...
          <div className="mt-6 pt-6 border-t">
            ...
            {recommendation.tradeoffs.length > 0 && (
              <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h5 className="font-semibold text-yellow-900 mb-2">Trade-offs to Consider</h5>
                <ul className="space-y-1">
                  {recommendation.tradeoffs.map((tradeoff, idx) => (
                    <li key={idx} className="text-sm text-yellow-800">• {tradeoff.description}</li>
                  ))}
                </ul>
              </div>
            )}
...
const ModelResultCard = ({ model, result, isRecommended }) => {
  ...
  return (
    <div className={`border rounded-lg p-4 ${isRecommended ? 'ring-2 ring-green-500 bg-green-50' : ''}`}>
      {isRecommended && (
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="font-medium">{(result.quality_score * 100).toFixed(0)}%</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-medium">${result.cost_per_request.toFixed(4)}</span>
        </div>
      </div>
    </div>
  );
      ...
      <tbody>
        {Object.entries(results).map(([model, result]) => (
          <tr key={model} className="border-b">
            ...
            <td className="py-2 text-right">{(result.quality_score * 100).toFixed(1)}%</td>
            <td className="py-2 text-right">{result.speed_ms.toFixed(0)}ms</td>
            <td className="py-2 text-right">${result.cost_per_request.toFixed(4)}</td>
            <td className="py-2 text-right">{(result.success_rate * 100).toFixed(1)}%</td>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Models to Test</label>
            <div className="space-y-2">
              {['gpt-4', 'gpt-3.5-turbo', 'claude-opus-4-20250514', 'claude-sonnet-4-20250514', 'claude-haiku-4-20250301'].map(model => (
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Test Prompts</label>
                ...
                {formData.test_prompts.length > 1 && (
                  <button
                    onClick={() => {
                      setFormData({
                        ...formData,
                        test_prompts: formData.test_prompts.filter((_, i) => i !== idx)
                      });
                    }}
            ...
            <button
              onClick={() => setFormData({ ...formData, test_prompts: [...formData.test_prompts, ''] })}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setFormData({ ...formData, criteria: formData.criteria.filter(c => c !== criterion) });
                      }
                    }}
                  />
        ...
        <div className="flex gap-2 mt-6">
          ...
          <button
            onClick={handleSubmit}
            disabled={!formData.name || formData.models.length < 2 || !formData.test_prompts[0]}
...
<head>
    ...
    <style>
        ...
        .metric { background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Your AI token quota is running low. You have <strong>{{ data.remaining | number_format }} tokens</strong> remaining ({{ data.percent | round(1) }}% of your quota).</p>
            
            <div class="metric">
                <strong>Current Usage</strong><br>
                {{ data.remaining | number_format }} / {{ data.total | number_format }} tokens remaining
            </div>
        ...
        <div class="footer">
            You're receiving this because you're using Alfred AI Governance Platform.<br>
...
<head>
    ...
    <style>
        ...
        .stat-card { background: #f9fafb; border: 1px solid #e5e7eb; padding: 20px; border-radius: 8px; text-align: center; }
        ...
        .team-list { background: white; border: 1px solid #e5e7eb; border-radius: 8px; }
        .team-member { padding: 15px; border-bottom: 1px solid #e5e7eb; display: flex; justify-content: space-between; }
    ...
    <div class="container">
        ...
        <div style="padding: 30px;">
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-value">{{ report.request_count | number_format }}</div>
            ...
            <div class="team-list">
                ...
                <div class="team-member">
                    ...
                    <span>{{ member.tokens | number_format }} tokens (${{ member.cost | round(2) }})</span>
                </div>
                {% endfor %}
            </div>
            ...
            <p style="margin-top: 30px;">
                <a href="https://alfred.company.com/dashboard/team" style="background: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">View Full Report</a>
            </p>
        </div>
    </div>
</body>
