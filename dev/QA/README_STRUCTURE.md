...
│   └── (Tests located in: dev/frontend/src/__tests__/)
...
**Note**: Frontend tests remain in `dev/frontend/src/__tests__/` (co-located with source code) for better module resolution and following React/Vite best practices.
...
| File | Purpose |
|------|---------|
| `pyproject.toml` | Pytest configuration, testpaths: `dev/QA/Backend` |
...
- `dev/frontend/src/__tests__/` → Remains in place (co-located with source)
- `dev/QA/Tests/` → `dev/QA/E2E/` (✅ Moved)

Old `backend/tests/` directorywas reorganized from:
- `backend/tests/` → `dev/QA/Backend/`
- `dev/frontend/src/__tests__/` → `dev/QA/Frontend/Unit/`
- `dev/QA/Tests/` → `dev/QA/E2E/`
