...
class TestSettings:
    ...
    def test_environment_validation(self):
        ...
        # Valid environments (non-production)
        ...
        settings = Settings(
            ...
            database_url="postgresql://user:pass@localhost/alfred"
        )
    ...
    def test_production_config_security(self):
        """Test that production configuration enforces security settings."""
        from pydantic import ValidationError
        from app.config import Settings
        
        # Production with wildcard CORS should be rejected
        with pytest.raises(ValidationError, match="CORS.*not allowed in production"):
            Settings(
                environment="production",
                cors_origins=["*"],
        ...
        # Production with SQLite should be rejected
        with pytest.raises(ValidationError, match="SQLite is not recommended for production"):
            Settings(
                environment="production",
        ...
        settings = Settings(
            environment="production",
            cors_origins=["https://alfred.example.com"],
            database_url="postgresql://user:pass@localhost/db",
            debug=False
        )
        assert settings.environment == "production"
        assert settings.cors_origins == ["https://alfred.example.com"]
        assert settings.debug is False
    
    def test_default_values_are_secure(self):
        """Test that default configuration values are secure for development."""
        from app.config import Settings
        
        # Development environment should have safe defaults
        settings = Settings(environment="development")
        
        # CORS should not be wildcard by default
        ...
        # API key should have secure prefix and length
        assert settings.api_key_prefix == "tp-"
