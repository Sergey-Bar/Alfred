...
class CreditCalculator:
    ...
    # Conversion factor: actual cost -> org credits (imported from constants)
...
class LLMProxy:
    ...
    async def forward_request(
        ...
        Includes automatic retry logic with exponential backoff (3 attempts max).
        Retries transient errors but not rate limits or auth failures.
        ...
        # Define retry wrapper with tenacity (if available)
        if TENACITY_AVAILABLE:
            ...
            async def _call_with_retry():
                return await completion(**kwargs, acompletion=True)
            
            response = await _call_with_retry()
        else:
            # Fallback without retry if tenacity not installed
