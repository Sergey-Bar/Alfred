...
from ..models import Wallet, User, WalletTransaction, WalletTransactionType
...
async def _handle_budget_command(
    ...
    # Look up user's wallet from DB by Slack user ID mapping
    with Session(get_engine()) as session:
        # Find user by Slack user ID (stored in metadata or external_id field)
        statement = select(User).where(User.slack_user_id == user_id)
        ...
        if not user:
            ...
            return Response(
                content=json.dumps({
                    "text": f"❌ No Alfred account found for Slack user {user_name}. Please contact your admin to link your account.",
                    "response_type": "ephemeral",
                }),
                media_type="application/json",
            )
        ...
        wallet_statement = select(Wallet).where(
        ...
        if not wallet:
            # No wallet found - return error
            return Response(
                content=json.dumps({
                    "text": f"❌ No wallet found for user {user_name}. Please contact your admin.",
                    "response_type": "ephemeral",
                }),
                media_type="application/json",
            )
        ...
        hard_limit = float(wallet.hard_limit) if wallet.hard_limit else 1000.00
        
        # TODO: Calculate actual spend stats from WalletTransaction table
        spend_today = 0.0
        spend_month = 0.0
        top_models = []
...
async def _handle_block_action(payload: Dict[str, Any]) -> Response:
    ...
    for action in actions:
        ...
        if action_id == "approve_budget":
            ...
            # Execute the actual wallet top-up via database transaction
            # Security: This should only execute after proper approval
            try:
                with Session(get_engine()) as session:
                    # Parse request data from request_id (format: "user_id:amount")
                   # In production, fetch from approval_requests table
                    # For now, log the action audit trail
                    wallet_transaction = WalletTransaction(
                        transaction_id=str(uuid.uuid4()),
                        wallet_id=None,  # TODO: Get from approval request
                        type=WalletTransactionType.CREDIT,
                        amount=Decimal("0"),  # TODO: Get from approval request
                        description=f"Slack approval by {approver_name}",
                        approved_by=approver_id,
                        created_at=datetime.now(timezone.utc),
                    )
                    # NOTE: Actual implementation requires:
                    # 1. Fetch approval request from DB by request_id
                    # 2. Validate approver has permission
                    # 3. Execute wallet top-up transaction
                    # 4. Update approval request status
                    # 5. Send confirmation notification
                    session.add(wallet_transaction)
                    session.commit()
                    logger.info(f"Wallet top-up executed for request {request_id}")
            except Exception as e:
                logger.error(f"Failed to execute wallet top-up: {e}")
...
async def _handle_view_submission(payload: Dict[str, Any]) -> Response:
    ...
    if callback_id == "alfred_budget_request":
        ...
        if approval_channel and SLACK_BOT_TOKEN:
            # Resolve team name from user mapping
            team_name = "Engineering"  # Default
            try:
                with Session(get_engine()) as session:
                    # Look up user by Slack ID
                    user_statement = select(User).where(User.slack_user_id == requester_id)
                    user = session.exec(user_statement).first()
                    if user and user.team_id:
                        # Get team name
                        from ..models import Team
                        team_statement = select(Team).where(Team.id == user.team_id)
                        team = session.exec(team_statement).first()
                        if team:
                            team_name = team.name
            except Exception as e:
                logger.warning(f"Failed to resolve team name for user {requester_id}: {e}")
...
async def slack_oauth_callback(code: str):
    ...
    # Store the bot token and team info in database for multi-workspace support
    bot_token = result.get("access_token")
    team_info = result.get("team", {})
    team_name = team_info.get("name", "Unknown")
    team_id = team_info.get("id", "")
    
    # Persist Slack workspace installation to database
    try:
        with Session(get_engine()) as session:
            ...
            workspace = SlackWorkspace(
                ...
                bot_access_token=bot_token,  # Encrypt in production!
                installed_at=datetime.now(timezone.utc),
                is_active=True,
            )
            
            # In production, check for existing and update, or use upsert
            session.add(workspace)
            session.commit()
            
            logger.info(f"Persisted Slack workspace installation: {team_name} ({team_id})")
    
    except Exception as e:
        # Log error but don't fail the OAuth flow
        logger.error(f"Failed to persist Slack workspace to DB: {e}")
        # NOTE: Bot token stored in-memory will work for single workspace,
        # but multi-workspace requires database persistence
