...
@router.get(
    "/admin/teams", response_model=List[TeamResponse], dependencies=[Depends(require_admin)]
)
...
@router.put(
    "/admin/teams/{team_id}", response_model=TeamResponse, dependencies=[Depends(require_admin)]
)
...
async def add_team_member_email(
    ...
    link = TeamMemberLink(team_id=team.id, user_id=user.id, is_admin=request.is_admin)
    session.add(link)
    session.commit()
...
async def add_team_member(
    ...
    link = TeamMemberLink(team_id=team.id, user_id=user.id, is_admin=is_admin)
    session.add(link)
    session.commit()
