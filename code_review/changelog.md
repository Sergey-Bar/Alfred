...
- **Logic:** Added interactive onboarding modal (`AdminOnboardingModal.jsx`) for new admin users, with step-by-step walkthrough of key features. Integrated onboarding launch button into UserGuide.jsx for contextual help/tutorial access.
- **Why:** The previous User Guide was static; admins needed a guided, actionable onboarding experience to reduce time-to-value and support burden.
- **Root Cause:** No interactive onboarding or contextual help for admins; only static documentation.
- **Context:**
  - New `AdminOnboardingModal.jsx` component provides a modal onboarding flow for admins.
  - UserGuide page now includes a button to launch onboarding at any time.
